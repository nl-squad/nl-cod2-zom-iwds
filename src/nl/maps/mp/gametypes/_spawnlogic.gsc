...
getSpawnpoint_NearTeam(spawnpoints)
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if(player.sessionstate == "spectator" || player.sessionstate == "dead" || player == self || (isDefined(player.pers["afk"]) && player.pers["afk"]) )
		{
			continue;
		}
	...
	if(isdefined(aliveplayers))
	{
		...
		for(i = 0; i < spawnpoints.size; i++)
		{
			...
			/* Do not respawn the same place as before */
			if(isdefined(self.lastspawnpoint) && self.lastspawnpoint == spawnpoints[i])
				continue;

			/* Do not respawn in ultimate fire */
			ok = 1;
			if(isDefined(level.ultimateFirePos) && level.ultimateFirePos.size > 0)
			{
				for(z = level.ultimateFirePos.size - 1; z >= 0; z -= 1)
				{
					if(level.ultimateFirePos[z] != (0, 0, 0) && distance(spawnpoints[i].origin, level.ultimateFirePos[z]) <= 100)
					{
						ok = 0;
					}
				}
			...
			{
				if(isDefined(aliveplayers[j].pers["team"]) && isDefined(self.pers["team"]))
				{
